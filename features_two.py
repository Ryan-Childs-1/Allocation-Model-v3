...
def _item_lifecycle_labels(l30, d30, d60_per30, lw_monthly, ttm_per30, qoh, supply) -> pd.Series:
    recent_avg = (l30 + d30 + lw_monthly) / 3.0
    labels = pd.Series('mature_mixed_item', index=l30.index, dtype='object')
    labels[(ttm_per30 <= 0) & (recent_avg > 0)] = 'new_active_item'
    labels[(ttm_per30 > 0) & (recent_avg <= 0)] = 'declining_or_dead_item'
    labels[(l30 > ttm_per30 * 1.5) & (d60_per30 >= ttm_per30) & (recent_avg > 0)] = 'seasonal_or_recovering_item'
    labels[(lw_monthly > recent_avg * 1.75) & (lw_monthly > 0)] = 'short_term_spike_item'
    labels[(recent_avg > 0) & (abs(l30 - d30) <= np.maximum(1.0, recent_avg * 0.25)) & (abs(d60_per30 - ttm_per30) <= np.maximum(1.0, recent_avg * 0.35))] = 'mature_consistent_item'
    labels[(recent_avg <= 0) & (ttm_per30 <= 0) & (supply > 0)] = 'stocked_no_demand_item'
    labels[(qoh <= 0) & (recent_avg > 0)] = 'out_of_stock_with_demand_item'
    return labels.fillna('mature_mixed_item')

def build_feature_frame(df: pd.DataFrame) -> pd.DataFrame:
    """Build model features in batched dictionaries to avoid DataFrame fragmentation.

    This v8 feature builder adds demand, lifecycle, store/item context, optimizer and reason-code support:
    ...
    6. item lifecycle and counterfactual-compatible signals.
    """
    ...
    lifecycle = _item_lifecycle_labels(l30, d30_per30, d60_per30, lw_monthly, ttm_per30, qoh, supply)
    cat['demand_pattern'] = pattern
    cat['item_lifecycle'] = lifecycle
    for name, metric in {
        'l30': l30,
        'd30_per30': d30_per30,
        'd60_per30': d60_per30,
    ...
    for life in ['new_active_item','mature_consistent_item','mature_mixed_item','declining_or_dead_item','seasonal_or_recovering_item','short_term_spike_item','stocked_no_demand_item','out_of_stock_with_demand_item']:
        num[f'item_lifecycle_{life}'] = (lifecycle == life).astype(float)
    num['lifecycle_new_or_recovering_signal'] = ((lifecycle == 'new_active_item') | (lifecycle == 'seasonal_or_recovering_item') | (lifecycle == 'out_of_stock_with_demand_item')).astype(float)
    num['lifecycle_decline_risk_signal'] = ((lifecycle == 'declining_or_dead_item') | (lifecycle == 'stocked_no_demand_item')).astype(float)
    ...
    num.update({
        ...
        'counterfactual_zero_dc_should_blank': ((dc_start <= 0) & (need_gap > 0)).astype(float),
        'counterfactual_supply_high_risk': (supply >= demand_basis + flm).astype(float),
        'counterfactual_alloc_rec_misleading_risk': ((alloc_rec > 0) & (need_gap <= 0)).astype(float),
        'counterfactual_low_dc_high_need': ((dc_start > 0) & (dc_start < need_gap) & (need_gap > 0)).astype(float),
    })
...
def build_targets(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Reason code target.  These are compact, business-readable classes:
    # 0 blank/no allocation, 1 demand-supported, 2 alloc-rec-followed,
    # 3 alloc-rec-reduced/zeroed, 4 alloc-above-rec/missed-by-rec,
    # 5 overstock-risk blank/cut, 6 review/manual-positive.
    reason = pd.Series(0, index=df.index, dtype='int64')
    reason[(final_alloc > 0) & (alloc_rec > 0) & (abs(units - alloc_rec_units) <= 1)] = 2
    reason[(final_alloc > 0) & ((alloc_rec <= 0) | (units > alloc_rec_units + 1))] = 4
    reason[(final_alloc > 0) & (reason == 0)] = 1
    reason[(alloc_rec > 0) & (final_alloc < alloc_rec)] = 3
    reason[(final_alloc <= 0) & (alloc_rec > 0) & (pd.to_numeric(X.get('num__counterfactual_supply_high_risk', 0), errors='coerce').fillna(0) > 0)] = 5
    reason[(section == 'review') & (final_alloc > 0)] = 6
    return pd.DataFrame({
        ...
        '__target_reason_code': reason.astype(int),
    }, index=df.index)
